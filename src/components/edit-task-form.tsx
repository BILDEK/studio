...
import { TaskComments, type Comment, type Attachment } from "@/components/task-comments"
...
interface EditTaskFormProps {
  ...
  onAddComment: (taskId: string, commentText: string, attachments: Attachment[]) => void; // Updated signature
  employees: Employee[]
  task: Task
  tasks: Task[]
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <FormField control={form.control} name="assigneeId" render={({ field }) => (<FormItem><FormLabel>Assign To</FormLabel><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Select an employee" /></SelectTrigger></FormControl><SelectContent>{employees.map((e) => (<SelectItem key={e.id} value={e.id}>{e.name}</SelectItem>))}</SelectContent></Select><FormMessage /></FormItem>)} />
                      ...
                      <ScrollArea className="h-24 w-full rounded-md border">
                        <div className="p-4">
                          {tasks.filter(t => t.id !== task.id).map((dependencyTask) => (
                              <div key={dependencyTask.id} className="flex flex-row items-start space-x-3 space-y-0 mb-2">
                                <Checkbox
                                  checked={field.value?.includes(dependencyTask.id)}
                                  onCheckedChange={(checked) => {
                                    const currentValues = field.value || [];
                                    if (checked) {
                                      field.onChange([...currentValues, dependencyTask.id]);
                                    } else {
                                      field.onChange(currentValues.filter(value => value !== dependencyTask.id));
                                    }
                                  }}
                                />
          ...
          <div className="space-y-6">
            ...
            <TaskComments 
                ...
                onAddComment={(commentText, attachments) => onAddComment(task.id, commentText, attachments)} // Pass attachments up
